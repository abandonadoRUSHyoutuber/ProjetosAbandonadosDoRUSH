...
public class AlunoProjetoController implements Serializable {
	...
	public void init(Pessoa pessoa) {
		try {
			...
			carregarLista(pessoa.getId().toString());
		} catch (Throwable e) {
			e.printStackTrace();
		}
	}
	...
	public void carregarLista(String id) {
		EntityManager em = JPAUtil.getEntityManager();
		lista = em.createNativeQuery("SELECT p.id, p.nome, p.professor_id, p.unidade_id                          "
			                       + "FROM   EquipeProjeto ep INNER JOIN projeto p      ON ep.projeto_id = p.id  "
			                       + "                        INNER JOIN EquipeAluno ea ON ea.id         = ep.id "
			                       + "WHERE " + id + " = ea.aluno_id                                             "
			                       , Projeto.class)
				.getResultList();
		em.close();
	}
