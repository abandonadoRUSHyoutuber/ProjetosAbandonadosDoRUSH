...
public class LeadCommand extends CommandHandler {
    ...
    public boolean noArgs(CommandSender sender, Command command, String label,
        ...
        // Header
        sender.sendMessage("§8§l[ §2§lR§aunas §8§l] §eLíderes em quantidade §7" + ChatColor.GRAY + (current + 1) + "§8:§7" + num);
        ...
        for(int i = current * LIMIT; i < (current * LIMIT + LIMIT); i++) {
            ...
            sender.sendMessage("§a" + (i + 1) + ") §e["+ Bukkit.getOfflinePlayer(UUID.fromString(player.getName())).getName() + "] §8- §a" + player.getPoints());
        }
