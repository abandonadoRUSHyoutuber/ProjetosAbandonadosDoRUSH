...
public class TruenoNPC_v1_8_r3 implements TruenoNPC {
		...
		if (jsonFile != null) {
			...
			while (it.hasNext()) {
				...
				if (element.getAsJsonObject().get("id").getAsInt() == this.npcid) {
					...
					// element.getAsJsonObject().addProperty("value",
					// skindata.getValue());
					// element.getAsJsonObject().addProperty("signature",
	...
	private void spawnEnttity(Player p, GameProfile gameprofile, SkinData skindata) {
		...
		try {
			...
			((Collection<String>) f.get(scbpacket)).add(profile.getName());
	...
	private void addToTablist(Player p) {
		...
		PacketPlayOutPlayerInfo.PlayerInfoData data = packet.new PlayerInfoData(gameprofile, 1,
				...
				CraftChatMessage.fromString("§8[NPC] " + gameprofile.getName())[0]);
		@SuppressWarnings("unchecked")
